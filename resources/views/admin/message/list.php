...
<div class="well form-well col-sm-12" style="margin-bottom: 15px">
  <form class="form-inline" id="search-form" role="form">
    ...
    <div class="pull-right">
      ...
      &nbsp;&nbsp;
      <a id="export-csv" class="btn btn-white" style="padding-top: 5px; padding-bottom: 5px" href="javascript:void(0);">导出</a>
    </div>
  </form>
</div>

<a class="new-message-tip alert alert-warning text-center display-none" href="javascript:;" style="margin:10px 0 0 0;padding:10px;">
...
<script id="user-head-img-tpl" type="text/html">
  <% if (source == '1') { %>
  <a class="pull-left user-popover" data-id="<%= user.id %>" target="_blank" href="<%= $.url('admin/message/user', {userId: user.id}) %>">
  ...
  <div class="media user-media">
    <div class="media-body">
      <h4 class="media-heading">
        <% if (source == '1') { %>
        <a href="<%= $.url('admin/message/user', {userId: user.id}) %>" target="_blank"><%= user.nickName || '&nbsp;' %></a>
        <% } else { %>
        <%= user.nickName || '&nbsp;' %>
        <% } %>
      </h4>
